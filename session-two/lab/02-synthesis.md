...
1. **Open `session-two/` as your workspace folder.** Codespaces opens the whole repo by default — use **File → Open Folder…** and pick `session-two/`. Every path in this walkthrough is relative to that folder, so the Explorer should show `from-previous-session/`, `inputs/`, `lab/`, and `solutions/` at its root. (Want to run locally instead? Clone `https://github.com/stephschofield/isd-vscode-training.git` and open the `session-two/` folder in VS Code.)
